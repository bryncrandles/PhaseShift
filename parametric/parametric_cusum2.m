function [critical_value, critical_value_lower, critical_value_upper] = parametric_cusum2(n_samples, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, phase_type, boundary, shift_magnitude, shift_latency, latency_tolerance)
...
while count_bootstraps < n_bootstrap && count_fails < max_fails
    ...
    if max_value > critical_value && abs(boundary + estimated_latency - shift_latency) < latency_tolerance
        ...
        statistics_lower(count_bootstraps) = max(abs(weighted_cusum(lower_phase)));
