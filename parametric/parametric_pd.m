function [critical_value, statistics] = parametric_pd(n_samples, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, phase_type, boundary)
% Parametric bootstrap procedure to estimating the critical value to use
% for change-point detection via PD statistic.
% 
% Inputs:
...
for boot = 1:n_bootstrap
    ...
    statistics(boot) = max(abs(pd(bootstrap_phase)));
end
