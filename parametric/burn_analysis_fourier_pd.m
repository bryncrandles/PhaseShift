...
for i = 1:n_burn_parameters
    ...
    [critical_value, statistic] = parametric_pd_fourier(signal_length, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, n_burn);    
    for j=1:n_shifts
        ...
        for k=1:n_simulations
            ...
            if max(abs(pd(phase))) > critical_value
...
save fourier_burnSize_pd true_positives n_simulations burn_list shift_list alpha n_samples SNR bandwidth 

% When the burn is less than 70, the null hypothesis is rejected, but it
% does not properly identify the latency. It always finds that the change
% point occurs at the end when the signal returns to zero.
