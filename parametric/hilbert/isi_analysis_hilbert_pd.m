...
[critical_value, ~, critical_value_upper] = parametric_pd2(n_samples, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, 'hilbert', boundary, shift_magnitude1, shift_latency1, latency_tolerance);
for i = 1:n_isi_levels
    shift_latency2 = shift_latency1 + isi_levels(i)
    for j = 1:n_shift_levels
        shift_magnitude2 = shift_levels(j)
        [~, critical_value_lower, ~] = parametric_pd2(n_samples, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, 'hilbert', boundary, shift_magnitude2, shift_latency2, latency_tolerance);
        for l = 1:n_simulations
            ...
            [max_value, estimated_latency] = max(abs(pd(phase)));
            if max_value > critical_value 
                ...
                elseif abs(boundary + estimated_latency - shift_latency2) < latency_tolerance
                    ...
                    [max_value_lower, estimated_latency_lower] = max(abs(pd(lower_phase)));
...
save isi_analysis_pd frequency bandwidth SNR shift_levels isi_levels n_simulations true_positives stage1_fails_A stage1_fails_B stage2_fails_A stage2_fails_B
