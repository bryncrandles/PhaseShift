function [critical_value, statistic] = parametric_pd_fourier_ISI(n_samples, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, n_burn, shift_magnitude, ISI)
...
base_critical_value = parametric_pd_fourier(n_samples, sampling_rate, SNR, frequency, bandwidth, alpha, n_bootstrap, n_burn);
...
for boot = 1:n_bootstrap
    ...
    [max_value, index] = max(abs(pd(bootstrap_phase)));
    if max_value > base_critical_value
        lower_phase = bootstrap_phase(1:(index - ISI));
        upper_phase = bootstrap_phase(1:(index + ISI));
        statistic(1, boot) = max(abs(pd(lower_phase)));
        statistic(2, boot) = max(abs(pd(upper_phase)));
    end
end
